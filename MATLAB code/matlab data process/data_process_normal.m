clear all;
tic;
inputFileName1 = input('input normal file name for samtools? *.txt/*.csv ', 's');
inputFileName2 = input('input normal file name for GATK? *.txt/*.csv ', 's');
outputFileName = input('output file name?  *.txt ', 's');


output = cell (1,30);%construct the outputFile!!!remember to modify
...
while (ischar(textLine1)==1)&&(ischar(textLine2)==1)
    ...
    elseif(token1{1,4}=='T')
       ...
       output{1,11} = refgenoIndex1;

    %the alternative genome #12
                altgeno1 = '';
    for k=1:G1 
                if(token1{1,4+k}=='X')
                    altgenoIndex = '0';
    ...
    for j = 1:tokenSize1(2);
        ...
        if j < 13
            ...
            if (strcmp(read1,'I16')==1)%set I1,2,5,6,9,10,13,14 8 components #3-10
                output(1,3) = token1(j+1);
                output(1,4) = token1(j+2);
                output(1,5) = token1(j+5);
                output(1,6) = token1(j+6);
                output(1,7) = token1(j+9);
                output(1,8) = token1(j+10);
                output(1,9) = token1(j+13);
                output(1,10) = token1(j+14);
            elseif (strcmp(read1,'PL')==1)%set PL 3 components   #13-15
                output{1,13} = token1{1,j+3};
                plmax = 0;
                plsum = 0;
            for k=2:3*G1;
                ...
                output{1,14} = num2str(plmax);
                output{1,15} = num2str(plsum);
            end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %for the GATK part
    
    %the QUAL from GATK #16
    output{1,16} = token2{1,5+G2};
    
    %set DP 1 compoment #18
    for j = 1:tokenSize2(2);
        read2 = token2(j);
        if j==tokenSize2(2)-3
            if (sum((read2{1,1}=='D')+(read2{1,1}=='P'))==2)
                output{1,18} = token2{j+3};
            end
        elseif j==tokenSize2(2)-8
            if (sum((read2{1,1}=='D')+(read2{1,1}=='P'))==2)
                output{1,18} = token2{j+1};
            end
        elseif j==tokenSize2(2)-11
            if (sum((read2{1,1}=='D')+(read2{1,1}=='P'))==2)
                output{1,18} = token2{j+7};
            end
        end
        
        
            
            if (sum((read2{1,1}=='A')+(read2{1,1}=='N'))==2)%set AN 1 component #17
                output(1,17) = token2(j+1);
            elseif (sum((read2{1,1}=='D')+(read2{1,1}=='e')+(read2{1,1}=='l')+(read2{1,1}=='s'))==4)%set Dels 1 compoment #19
                output(1,19) = token2(j+1); 
            elseif (strcmp(read2,'HaplotypeScore')==1)%set HaplotypeScore 1 compoment #20
                output(1,20) = token2(j+1);
            elseif (strcmp(read2,'MQ')==1)%set MQ 1 compoment #21
                output(1,21) = token2(j+1);    
            elseif (strcmp(read2,'MQ0')==1)%set MQ0 1 compoment #22
                output(1,22) = token2(j+1);
            elseif (sum((read2{1,1}=='Q')+(read2{1,1}=='D'))==2)%set QD 1 compoment #23
                output(1,23) = token2(j+1);
            elseif (sum((read2{1,1}=='S')+(read2{1,1}=='B'))==2)%set SB 1 compoment #24
                output(1,24) = token2(j+1);  
            elseif (sum((read2{1,1}=='G')+(read2{1,1}=='Q'))==2)%set GQ 1 compoment #25
                output(1,25) = token2(j+7);
            elseif (strcmp(read2,'PL')==1)%set PL G components #28-30  
            for k=1:3;
                output(1,27+k) = token2(j+6+k);
            end
            end
     end
            %the reference genome #26
       ...
       output{1,26} = refgenoIndex2;
            %the alternative genome #27
             altgeno2 = '';
    for k=1:G2
